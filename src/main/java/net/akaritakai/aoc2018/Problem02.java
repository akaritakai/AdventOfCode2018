...
public class Problem02 extends AbstractProblem {
  ...
  public String solvePart2() {
    for (final var input1 : getInput()) {
      for (final var input2 : getInput()) {
  ...
  private boolean characterAppearsNTimes(@NotNull final String s, final int n) {
    return characterFrequencyMap(s).values().stream().anyMatch(count -> count == n);
  }
  ...
  private List<String> getInput() {
    return getPuzzleInput().lines().collect(Collectors.toList());
  }
}
