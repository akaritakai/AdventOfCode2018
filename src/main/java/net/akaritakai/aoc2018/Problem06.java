...
public class Problem06 extends AbstractProblem {
  ...
  public String solvePart1() {
    ...
    final var maxWidth = points.stream().max(Comparator.comparingInt(p -> p.x)).map(point -> point.x).orElseThrow();
    final var maxHeight = points.stream().max(Comparator.comparingInt(p -> p.y)).map(point -> point.y).orElseThrow();
    ...
    final var largestRegion = regions.values().stream().max(Long::compare);
  ...
  /**
   * Gets the size of the region which have a total distance to all coordinates in the puzzle input by the provided
   * amount. Exposed for test purposes.
   */
  public String solvePart2(final int distance) {
    // Fetch our puzzle input as a list of points
    final var points = getPoints();
    ...
    for (var x = 0; x <= maxWidth; x++) {
      for (var y = 0; y <= maxHeight; y++) {
        final var point = new Point(x, y);
        final var distanceSum = points.stream().mapToLong(p -> distance(p, point)).sum();
  ...
  private Point findClosestPoint(@NotNull final Point point, @NotNull final List<Point> allPoints) {
    ...
    final var closestPoint = allPoints.stream().min(Comparator.comparingLong(p -> distance(p, point))).orElseThrow();
  ...
  private List<Point> getPoints() {
    return getPuzzleInput().lines().map(line -> {
