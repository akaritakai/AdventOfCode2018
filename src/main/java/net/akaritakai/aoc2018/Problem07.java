...
public class Problem07 extends AbstractProblem {
  ...
  public String solvePart2() {
    return solvePart2(5, 60);
  }

  /**
   * Solves part 2 of the problem given a number of workers and an extra amount of time needed to complete each task.
   * Exposed for test purposes.
   */
  public String solvePart2(final int numWorkers, final int baseTime) {
    ...
    while (completedNodes.size() < allNodes.size()) {
      ...
      for (var task : available) {
        for (var i = 0; i < numWorkers; i++) {
          if (timeLeft[i] <= 0) {
            assignments[i] = task;
            timeLeft[i] = timeRequired(task, baseTime);
            break;
          }
        }
      }
  ...
  /**
   * Gets the list of tasks available to be worked on, if any.
   */
   ...
   */
  private int timeRequired(@NotNull final String task, final int baseTime) {
    return baseTime + task.charAt(0) - 'A' + 1;
  }
